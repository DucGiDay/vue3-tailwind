<template>
  <Dialog
    ...
    :pt="{
      content: {
        class: '!fb-overflow-hidden',
        style: 'display: flex; flex-direction: column; min-height: 0; padding: 0;'
      },
      header: {
        class: 'fb-cursor-move !fb-p-0 !fb-border-b-0'
      },
      // root: { style: 'resize: both; overflow: auto; min-width: 300px; min-height: 200px;' }
      root: { style: 'position: relative; min-width: 50vw; min-height: 60vh; overflow: hidden;' }
    }"
    @hide="handleHide"
  >
    <template #header>
    ...
    <div class="fb-flex fb-flex-1 fb-min-h-0 fb-overflow-hidden">
      ...
      <div class="fb-flex-1 fb-flex fb-flex-col fb-min-h-0 fb-bg-white">
        ...
        <div class="fb-flex-1 fb-min-h-0 fb-overflow-y-auto fb-px-5 fb-pt-6 fb-pb-4">
...
const { dialogWidth, dialogHeight, startResizeLeft, startResizeTop, startResizeCorner } = useDialogResize(400, 200)
...
// });
const dialogStyle = computed(() => {
  if (dialogWidth.value || dialogHeight.value) {
    return {
      width: dialogWidth.value ? dialogWidth.value + 'px' : `min(92vw, calc(50vw))`,
      height: (dialogHeight.value || 600) + 'px',
    }
  }
